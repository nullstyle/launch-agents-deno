...
Deno.test("write is atomic, idempotent, and protects different files", async () => {
...
Deno.test("remove deletes the plist and reports absence", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("install uses modern GUI domain targets", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("install with load: false only writes the plist", async () => {
  await withTempDirectory(async (directory) => {
    const runner = new FakeRunner();
    const agents = new LaunchAgents({ directory, uid: 501, runner });
...
Deno.test("install is a no-op for a loaded, unchanged agent", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("install with reloadLoaded: false leaves the old definition running", async () => {
  await withTempDirectory(async (directory) => {
    const runner = new FakeRunner([success]);
    const agents = new LaunchAgents({ directory, uid: 501, runner });
    await agents.write(definition());
...
Deno.test("install refuses to replace a different file without overwrite", async () => {
  await withTempDirectory(async (directory) => {
    const runner = new FakeRunner();
    const agents = new LaunchAgents({ directory, uid: 501, runner });
    await agents.write(definition());
...
Deno.test("a raced creation surfaces LaunchAgentFileExistsError unwrapped", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("a failed reload restores the previous plist and service", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("rollback does not re-bootstrap when the service was not loaded from this plist", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("rollback does not re-bootstrap when the previous plist cannot be restored", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("uninstall boots out a loaded service and removes its plist", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("uninstall tolerates a missing plist and unloaded service by default", async () => {
  await withTempDirectory(async (directory) => {
...
Deno.test("uninstall with ignoreMissing: false requires the plist to exist", async () => {
  await withTempDirectory(async (directory) => {
    const runner = new FakeRunner();
    const agents = new LaunchAgents({ directory, uid: 501, runner });
...
Deno.test("uninstall with resetDisabled clears the persistent disabled state", async () => {
  await withTempDirectory(async (directory) => {
    const runner = new FakeRunner([notFound, success]);
    const agents = new LaunchAgents({ directory, uid: 501, runner });
    await agents.write(definition());
...
Deno.test("uninstall restores the booted-out service when removal fails", async () => {
  await withTempDirectory(async (directory) => {
    const path = `${directory}/dev.example.test-agent.plist`;
...
Deno.test("uninstall reports a rollback double fault as an AggregateError", async () => {
  await withTempDirectory(async (directory) => {
    const path = `${directory}/dev.example.test-agent.plist`;
    const runner = new FakeRunner([success, success, ioFailure], (args) => {
      if (args[0] === "bootout") Deno.removeSync(path);
...
Deno.test("lifecycle helpers pass literal arguments without a shell", async () => {
  const runner = new FakeRunner([
    { code: 0, stdout: "4242\n", stderr: "" },
    { code: 0, stdout: "4343\n", stderr: "" },
    success,
...
Deno.test("enable, disable, bootout, and bootstrap target the configured domain", async () => {
...
Deno.test("isLoaded distinguishes not-found from other launchctl failures", async () => {
  const runner = new FakeRunner([
    success,
    notFound,
...
Deno.test("inspect and kickstart failures preserve the launchctl invocation", async () => {
  const runner = new FakeRunner([
    ioFailure,
...
Deno.test("stop validates the signal before invoking launchctl", async () => {
  const runner = new FakeRunner([success, success]);
  const agents = new LaunchAgents({ directory: "/nonexistent/agents", uid: 501, runner });
...
Deno.test("a configured timeout passes an abort signal to the runner", async () => {
  const runner = new FakeRunner([success]);
  const agents = new LaunchAgents({
...
Deno.test("constructor and label validation reject malformed input", () => {
  const runner = new FakeRunner();
