...
describe("LaunchAgentBuilder", () => {
  it("builds a definition equal to the hand-written literal, field by field", () => {
  ...
  it("field-named methods replace the whole field, last call wins", () => {
  ...
  it("add* methods accumulate in call order without filtering", () => {
  ...
  it("addCalendarIntervals normalizes a single stored interval to array form", () => {
  ...
  it("sugar desugars onto exactly one modeled field", () => {
  ...
  it("build() reports the validator's own issues, unfiltered", () => {
  ...
  it("accumulators pass unknown keys through to the validator", () => {
  ...
  it("builders are frozen, immutable templates", () => {
  ...
  it("from() seeds a builder that round-trips and extends a definition", () => {
  ...
  it("a cast-defeated compile gate still fails at build() via the validator", () => {
