...
import { assert, assertEquals, assertIncludes, assertThrows } from "./assert.ts";

Deno.test("renders an idiomatic definition as a complete launchd plist", () => {
  ...
  assertIncludes(xml, "<!DOCTYPE plist PUBLIC");
  assertIncludes(xml, "<key>Label</key>\n    <string>dev.example.backup</string>");
  assertIncludes(xml, "<string>one &amp; two</string>");
  assertIncludes(xml, "<key>StartCalendarInterval</key>\n    <array>");
  assertIncludes(xml, "<key>ProcessType</key>\n    <string>Background</string>");
  assertEquals(xml.endsWith("\n"), true);
});

Deno.test("maps nested launchd structures without mutating the input", () => {
  const config: LaunchAgentConfig = {
...
Deno.test("validation reports multiple actionable paths", () => {
...
Deno.test("escapes XML in keys and values and renders empty collections", () => {
  ...
  assertIncludes(xml, "<key>A&amp;B&lt;C&gt;&quot;D</key>");
  assertIncludes(xml, "<string>x &lt; y &gt; z &amp; &quot;q&quot;</string>");
  assertIncludes(xml, "<array/>");
  assertIncludes(xml, "<dict/>");
});

Deno.test("rendering is deterministic across insertion orders", () => {
  ...
  assertIncludes(first, "<key>Alpha</key>");
});

Deno.test("umask renders as a decimal integer or an octal string", () => {
  const base = { label: "dev.example.umask", program: "/usr/bin/true" } as const;

  assertIncludes(renderLaunchAgent({ ...base, umask: 18 }), "<integer>18</integer>");
  assertIncludes(renderLaunchAgent({ ...base, umask: "022" }), "<string>022</string>");
...
Deno.test("validation enforces boundary values", () => {
  ...
  const rejected: [string, LaunchAgentConfig, string][] = [
    ...
    ["month zero", { ...base, startCalendarInterval: { month: 0 } }, "startCalendarInterval.month"],
  ...
  assertIncludes(xml, "<string>🚀</string>");
  assertIncludes(xml, "<integer>0</integer>");
});

Deno.test("defineLaunchAgent validates and returns the definition unchanged", () => {
...
Deno.test("validateLaunchAgent narrows unknown input", () => {
...
Deno.test("extra keys cannot override modeled keys", () => {
  const error = assertThrows(
    () =>
      renderLaunchAgent({
